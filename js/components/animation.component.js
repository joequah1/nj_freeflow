...
myComponent.prototype.animateProperty = function () {
    ...
    this.transitionIn = ['fadeIn', 'slideLeftIn', 'slideRightIn', 'flipXIn', 'flipYIn'];
    this.transitionOut = ['fadeOut', 'slideLeftOut', 'slideRightOut', 'flipXOut', 'flipYOut'];
    ...
    var outDelay = this.sdk.getInput({
        'title' : { 'title' : 'Delay' },
        'id' : '',
        input : {
            'type' : 'number',
    ...
    var inDuration = this.sdk.getInput({
    ...
    var outDuration = this.sdk.getInput({
        'title' : { 'title' : 'Duration' },
        'id' : '',
        input : {
            'type' : 'number',
    ...
    var play = this.sdk.getButton({
        'title' : {'title' : '' },
        'input' : { value : 'play', tab : this.tab, style : 'margin-top: 10px;' },
        'event' : {
            'type' : 'click',
            'callback' : function () {
    ...
    this.animationContainer.append([inSelection, inDelay, inDuration, divider, outSelection, outDelay, outDuration, play]);   
    
    //this.register();
}

myComponent.prototype.changeAnimationProperty = function (tab, index) {
    /* set in input */
    $('.ff_animation_in_transition[tab=' + tab + ']').val(this.data.layers[index].buildIn.transition);
    $('.ff_animation_in_delay_input[tab=' + tab + ']').val(this.data.layers[index].buildIn.delay / 1000);
    $('.ff_animation_in_duration_input[tab=' + tab + ']').val(this.data.layers[index].buildIn.duration / 1000);
    /* set out input */
    $('.ff_animation_out_transition[tab=' + tab + ']').val(this.data.layers[index].buildOut.transition);
    $('.ff_animation_out_delay_input[tab=' + tab + ']').val(this.data.layers[index].buildOut.delay / 1000);
    $('.ff_animation_out_duration_input[tab=' + tab + ']').val(this.data.layers[index].buildOut.duration / 1000);
}
...
myComponent.prototype.register = function () {
    ...
    for ( var fn in this.components ) {   
        ...
        //console.log(fn);
        console.log(this.components);
        //if (typeof this.components[fn] == 'function') {
            console.log(this);
            //var t = new this.components[fn](this);
            console.log(this.components);
            //this.components[fn] = {};
        //}
    }
    console.log(this.tab);
    //this.components['animation'].A();
}
myComponent.prototype.component = function (name, fn) {
    this.components = this.components || {};
    this.components[name] = fn;
}
myComponent.prototype.component('animation', function (_this) {
    var _this = _this;
    return {
        A : function () { _this.test2(); },
        B : function () { _this.test2(); }
    };
});
myComponent.prototype.test2 = function () {console.log(this.tab);}